...
pub trait LookupFilesApi: Send + Sync {
    ...
    ///
    /// 
    async fn create_file_v1(&self,  params: CreateFileV1Params ) -> Result<models::DomainLookupFileWrapper, Error<CreateFileV1Error>>;

    /// PATCH /loggingapi/entities/lookup-files/v1
    ///
    /// 
    async fn update_file_v1(&self,  params: UpdateFileV1Params ) -> Result<models::DomainLookupFileWrapper, Error<UpdateFileV1Error>>;
}
...
impl LookupFilesApi for LookupFilesApiClient {
    async fn create_file_v1(&self,  params: CreateFileV1Params ) -> Result<models::DomainLookupFileWrapper, Error<CreateFileV1Error>> {
        ...
        let local_var_uri_str = format!("{}/loggingapi/entities/lookup-files/v1", local_var_configuration.base_path);
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{local_var_unknown_type}` content type response that cannot be converted to `models::DomainLookupFileWrapper`")))),
            }
        } else {
            let local_var_entity: Option<CreateFileV1Error> = serde_json::from_str(&local_var_content).ok();
    ...
    async fn update_file_v1(&self,  params: UpdateFileV1Params ) -> Result<models::DomainLookupFileWrapper, Error<UpdateFileV1Error>> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            ...
            let local_var_entity: Option<UpdateFileV1Error> = serde_json::from_str(&local_var_content).ok();
            let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
            Err(Error::ResponseError(local_var_error))
        }
    }
