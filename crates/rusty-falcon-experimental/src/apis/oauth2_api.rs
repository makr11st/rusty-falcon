...
pub trait Oauth2Api: Send + Sync {
    ...
    ///
    /// 
    async fn oauth2_access_token(&self,  params: Oauth2AccessTokenParams ) -> Result<models::DomainAccessTokenResponseV1, Error<Oauth2AccessTokenError>>;

    /// POST /oauth2/revoke
    ///
    /// 
    async fn oauth2_revoke_token(&self,  params: Oauth2RevokeTokenParams ) -> Result<models::MsaspecResponseFields, Error<Oauth2RevokeTokenError>>;
}
    ...
    async fn oauth2_access_token(&self,  params: Oauth2AccessTokenParams ) -> Result<models::DomainAccessTokenResponseV1, Error<Oauth2AccessTokenError>> {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{local_var_unknown_type}` content type response that cannot be converted to `models::DomainAccessTokenResponseV1`")))),
            }
        } else {
