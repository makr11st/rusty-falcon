...
pub trait IntelligenceIndicatorGraphApi: Send + Sync {
    ...
    async fn lookup_indicators(
        &self,
        params: LookupIndicatorsParams,
    ) -> Result<models::RestapiLookupIndicatorResponse, Error<LookupIndicatorsError>>;
...
impl IntelligenceIndicatorGraphApi for IntelligenceIndicatorGraphApiClient {
    ...
    async fn lookup_indicators(
        ...
        let LookupIndicatorsParams { body } = params;
            ...
            match local_var_content_type {
                ...
                ContentType::Text => {
                    return Err(Error::from(serde_json::Error::custom(
                        "Received `text/plain` content type response that cannot be converted to `models::RestapiLookupIndicatorResponse`",
                    )));
                }
                ContentType::Unsupported(local_var_unknown_type) => {
                    return Err(Error::from(serde_json::Error::custom(format!(
                        "Received `{local_var_unknown_type}` content type response that cannot be converted to `models::RestapiLookupIndicatorResponse`"
                    ))));
                }
            }
        } else {
            let local_var_entity: Option<LookupIndicatorsError> =
    ...
    async fn search_indicators(
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => {
                    return Err(Error::from(serde_json::Error::custom(format!(
                        "Received `{local_var_unknown_type}` content type response that cannot be converted to `models::RestapiIndicatorResponse`"
                    ))));
                }
            }
        } else {
