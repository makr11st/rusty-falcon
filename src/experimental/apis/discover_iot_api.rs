...
impl DiscoverIotApi for DiscoverIotApiClient {
    async fn get_iot_hosts(
        ...
        let GetIotHostsParams { ids } = params;
            ...
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => {
                    return Err(Error::from(serde_json::Error::custom(format!(
                        "Received `{local_var_unknown_type}` content type response that cannot be converted to `models::DomainDiscoverApiioTHostEntitiesResponse`"
                    ))));
                }
            }
        } else {
