...
impl DetectsApi for DetectsApiClient {
    ...
    async fn get_aggregate_detects(
        ...
        let GetAggregateDetectsParams { body } = params;
    ...
    async fn get_detect_summaries(
        ...
        let GetDetectSummariesParams { body } = params;
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => {
                    return Err(Error::from(serde_json::Error::custom(format!(
                        "Received `{local_var_unknown_type}` content type response that cannot be converted to `models::DomainMsaDetectSummariesResponse`"
                    ))));
                }
            }
        } else {
    ...
    async fn update_detects_by_ids_v2(
        ...
        let UpdateDetectsByIdsV2Params { body } = params;
